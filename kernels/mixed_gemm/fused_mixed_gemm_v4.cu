/***************************************************************************************************
 * Fused Mixed-Precision GEMM v4 — No workspace, FP16 register staging
 *
 * Improvement over v3: eliminates workspace global memory traffic.
 *   Phase 1: WGMMA → acc(INT32) → dequant → FP16 register fragment (kept live)
 *   Phase 2: clear(acc), WGMMA → acc(INT32) → dequant + combine with FP16 fragment → store
 *
 * Register budget: 128 INT32 acc + 128 FP16 phase1 results = 192 regs/thread
 * (vs v2's 256 regs with dual INT32 acc, vs v3's 128 regs + global workspace IO)
 *
...
static constexpr uint32_t MaxThreadsPerBlock_v4 = GemmKernel::MaxThreadsPerBlock;

// =============================================================================
// Params
// =============================================================================

struct FusedV4Params {
...
__global__ void __launch_bounds__(MaxThreadsPerBlock_v4, 1)
fused_gemm_kernel_v4(__grid_constant__ FusedV4Params const params) {
    ...
    // FP16 register fragment to hold Phase 1 dequant result (stays live across phases)
    auto phase1_fp16 = make_fragment_like<ElementOutput>(acc);
    ...
    clear(acc);
    {
        ...
        if (warp_group_role == WarpGroupRole::Producer) {
            ...
            // Dequant main → store in phase1_fp16 register fragment
            CUTLASS_PRAGMA_UNROLL
            for (int i = 0; i < size(acc); ++i) {
                ...
                phase1_fp16(i) = ElementOutput(result);
            }
        }
    }
    ...
    if (k_tile_count_high > 0) {
        ...
        if (warp_group_role == WarpGroupRole::Producer) {
            ...
            // Dequant high + combine with Phase 1 register fragment + store
            CUTLASS_PRAGMA_UNROLL
            for (int i = 0; i < size(acc); ++i) {
                auto coord = tCcD(i);
                int m_g = tile_m_start + get<0>(coord);
                ...
                // Combine with Phase 1 result from register (no global memory!)
                float out_main = float(phase1_fp16(i));
        ...
        // No high phase — store Phase 1 FP16 fragment directly
        if (warp_group_role == WarpGroupRole::Consumer) {
            CUTLASS_PRAGMA_UNROLL
            for (int i = 0; i < size(phase1_fp16); ++i) {
                auto coord = tCcD(i);
                int m_g = tile_m_start + get<0>(coord);
                int n_g = tile_n_start + get<1>(coord);
                if (m_g >= params.M || n_g >= params.N) continue;
                params.D[m_g * params.ldd + n_g] = phase1_fp16(i);
            }
        }
    }
#endif
    ...
    torch::Tensor s_x_h, torch::Tensor s_w_h, torch::Tensor neg_zero_h, torch::Tensor colsum_h)
{
    ...
    dim3 block(MaxThreadsPerBlock_v4, 1, 1);

    int smem_size = static_cast<int>(sizeof(typename GemmKernel::SharedStorage));
    auto err = cudaFuncSetAttribute(fused_gemm_kernel_v4,
        cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    TORCH_CHECK(err == cudaSuccess, "cudaFuncSetAttribute failed: ", cudaGetErrorString(err));

    fused_gemm_kernel_v4<<<grid, block, smem_size, stream>>>(kernel_params);

    err = cudaGetLastError();
    TORCH_CHECK(err == cudaSuccess, "fused_gemm_kernel_v4 launch failed: ", cudaGetErrorString(err));
