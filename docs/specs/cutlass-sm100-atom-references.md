...
- **Round 20** (2026-06-24): closed Codex round-19-review's blocking side issue on memo self-consistency. The round-19 fix corrected the MXFP8 trait-table M-dimension row (line 31) but missed the parallel NVFP4 row (line 67), which still said "M ∈ {64, 128}" / "same as MXFP8 atom". Round 20 corrected the NVFP4 M-dimension row to cite `mma_sm100_umma.hpp:1616` (M=128 for 1-CTA) and `mma_sm100_umma.hpp:1758` (M ∈ {128, 256} for 2x1SM). The parallel N-dimension rows in both tables (lines 32 and 68) were also normalized to cite the atom-level static_assert form `N % 8 == 0 ∧ 8 ≤ N ≤ 256`; the round-1 "stride 16 if B is MN-major" subclause is now noted as caller-side B-major handling, not atom-level. Codex round-20 review accepted task2 as Completed and Verified after this fix.
