...
### 8.1 PCA 低方差通道的数据范围问题

**问题**: PCA 投影后，低方差维度的数据波动小，但**均值**不受方差约束。

假设原始激活 x 的均值是 μ，PCA 变换后 z = x·U，则 E[z_i] = μ·u_i（第 i 个特征向量的投影），**不一定为零**。

因此完全可能出现低方差维度之间均值差距很大的情况：
```
低方差维度1: 均值=10, 方差=0.01 → 数据集中在 [9.8, 10.2]
低方差维度2: 均值=20, 方差=0.01 → 数据集中在 [19.8, 20.2]
低方差维度3: 均值=-5, 方差=0.02 → 数据集中在 [-5.3, -4.7]
```

若对这些通道做 per-tensor 量化，量化范围变成 [-5.3, 20.2]，而每个通道的实际信息量只占 0.4/25.5 ≈ 1.6%，精度极度浪费。

ResQ 的保护措施：
1. **非对称量化** (`--a_asym`)：计算 zero_point 处理偏移
2. **per-token 量化** (groupsize=-1)：每个 token 独立计算 min/max

但即使如此，同一 token 内低方差维度间的均值差距仍然浪费量化范围。这正是 Hadamard 旋转要解决的问题。

### 8.2 Hadamard 旋转原理

Hadamard 矩阵 H_n 是 n×n 正交矩阵，所有元素只有 ±1/√n：

```
H_2 = (1/√2) * [[1, 1], [1, -1]]
```

递归构造：H_{2n} = (1/√2) * [[H_n, H_n], [H_n, -H_n]]

**关键性质**：
- **正交变换**: H·H^T = I，完全可逆，**数学上零损失**
- **均匀混合**: 每个输出维度是所有输入维度的等权(±1/√n)叠加

**直觉**：Hadamard 变换是"完美搅拌器"：
```
变换前:  [1.0, 1.0, 1.0, 100.0]  ← outlier 独占一个通道
                ↓ Hadamard
变换后:  [25.75, -24.25, -24.25, 25.75]  ← outlier 能量均匀分散
```

变换前量化分辨率 = 100/15 ≈ 6.67，变换后 = 50/15 ≈ 3.33，精度提升一倍。

**Hadamard 本身无损耗**。正交变换保持所有信息。损耗只发生在之后的量化步骤，但因为分布变均匀了，量化误差反而更小。

### 8.3 为什么不对所有通道统一做 Hadamard？

这正是 QuaRot/SpinQuant 的做法：全部通道 Hadamard → 统一 4-bit。效果不错但 ResQ 声称更好。原因：

**Hadamard 太均匀了——消除了区分重要性的能力。**

旋转后所有维度看起来一样，只能给相同 bit 数。但信息论的 rate-distortion 理论表明：**给定总比特预算，对重要信息分配更多比特才能最小化总失真**。

数值例子（K=2048，高方差 256 维总能量 800，低方差 1792 维总能量 200）：

```
方案A：全 Hadamard → 全 4-bit
  每维能量 = 1000/2048 ≈ 0.488
  总误差 = 2048 × 0.488/16² = 3.91

方案B：PCA分组 → 组内 Hadamard → 高方差 8-bit, 低方差 4-bit
  高方差: 256 × 3.125/256² = 0.0122
  低方差: 1792 × 0.1116/16² = 0.781
  总误差 = 0.793

ResQ 总误差 0.793 vs 全 Hadamard 3.91，差 ~5 倍。
```

**本质：PCA 负责"分组"，Hadamard 负责"组内均匀化"，两者缺一不可。**

### 8.4 高方差组也需要 Hadamard 的原因

高方差组（1/8 通道，8-bit）内部，不同维度的方差仍可能差距很大。PCA 特征值降序排列后，组内 max/min 可达 200x+。Hadamard 旋转将组内方差均匀化，确保 8-bit 精度被均匀分配。

高方差 → 旋转收益大（而非损耗大）：方差差异越大，均匀化效果越明显，量化收益越大。

### 8.5 不同 GEMM 的最优 high_fraction 分析

**核心发现：ResQ 统一使用 1/8 并非最优。**

我们从 Llama-3.2-1B 的 PCA 特征值文件（E-wikitext-512-Llama-3.2-1B.bin）中实际分析了各模块的特征值分布。

#### 8.5.1 attn_mlp 共享特征值 (K=2048)

```
特征值范围: [0.196, 256.6], max/min = 1309x
  rank 0:   λ=256.6  (单独占总方差 12.4%)
  rank 1:   λ=76.1
  rank 2:   λ=52.6
  rank 256: λ=1.18   ← 这是 1/8 切分点
  rank 2047: λ=0.196
```

各 fraction 对应的高方差组占总方差：
...
| 模块 | K | top1/8占比 | max/min | 分布特征 |
|------|:-:|:-:|:-:|:-:|
| attn_mlp (shared) | 2048 | 60.7% | 1309x | 极度不均匀，头部集中 |
| L0 down_proj | 256 | 30.4% | 21x | 相对均匀 |
| L7 down_proj | 256 | 26.0% | 16x | 相对均匀 |
| L15 down_proj | 256 | 54.8% | 108x | 很不均匀（最后一层最极端）|
| L0 value (avg heads) | 64 | 44.3% | 45x | 中等不均匀 |
| L5 value (avg heads) | 64 | 51.9% | 70x | 较不均匀 |
| L11 value (avg heads) | 64 | 33.3% | 17x | 相对均匀 |
| key_pos (各层) | 64 | 54-61% | 35-178x | 一致地不均匀 |

#### 8.5.3 结论

1. **不同模块的最优 fraction 不同**：L0-L7 的 down_proj（top1/8 仅占 26-30%）可能不需要分组或需要更大 fraction；L15 的 down_proj（54.8%）可能需要更小的 fraction（1/16）
2. **full_shared 模式本身是粗糙近似**：所有层的 q/k/v/gate/up_proj 共享 PCA basis，但各层激活分布差异显著
3. **自适应 fraction 是潜在改进方向**：为每个模块根据特征值分布自动选择切分点，可在不增加平均比特预算的前提下降低量化误差
4. **对 kernel 设计的启示**：混合精度 GEMM kernel 需要支持可变的高/低精度比例，而非硬编码 1/8
