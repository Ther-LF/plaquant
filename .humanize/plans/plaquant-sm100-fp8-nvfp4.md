...
- **AC-1: Quantization Format Specification (Single Source of Truth)**
  ...
  - Positive Tests:
    ...
    - **Rounding mode**: round-to-nearest-even (ties-to-even); applied at element quantization step. Block scale itself is computed by `block_scale = max_abs(block) / max_format_value`, then rounded down to the nearest representable scale value (E8M0 = power-of-2; E4M3 = nearest representable FP8).
...
3. **M2 — Fused SM100 kernel + characterization (gating: AC-3, AC-4, AC-5)**
   ...
   - Phase D: Shape-sweep `benchmark.py` with three-column output (fused/2-launch/FP16) + per-stage profile
