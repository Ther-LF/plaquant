    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.R2_1 is not None:
            R2 = torch.block_diag(self.R2_1.weight, self.R2_2.weight)
            if self.R2_0 is not None:
                R2 = torch.block_diag(self.R2_0.weight, R2)
        query_states = self.q_proj(hidden_states, R1)
        key_states = self.k_proj(hidden_states, R1)
        value_states = self.v_proj(hidden_states, R1, R2=R2)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if self.R1_1 is not None:
            R1 = torch.block_diag(self.R1_1.weight, self.R1_2.weight)
            if self.R1_0 is not None:
                R1 = torch.block_diag(self.R1_0.weight, R1)
        else:
            R1 = None
        ...
        if R1 is not None:
            ...
            hidden_states = (
                hidden_states.to(torch.float64) @ R1.T.to(torch.float64)
            ).to(dtype)
        if self.config.pretraining_tp > 1:
